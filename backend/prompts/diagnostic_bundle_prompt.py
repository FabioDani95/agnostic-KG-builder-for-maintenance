...
The input contains physical page markers and opaque [[EVIDENCE_ID: ...]] anchors.
...
5. A CorrectiveAction is an explicitly restorative remedy for that failure. Checks and tests
   are not restorative unless the source says the check itself resolves the fault.
...
9. Every quote is a short verbatim span contained in the EvidenceUnit named by source_anchor,
   on source_page. Copy the opaque anchor exactly. Do not include page/anchor marker text in a
   quote. Evidence for different edges may come from different anchors or adjacent pages.
