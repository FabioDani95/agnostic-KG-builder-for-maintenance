...
from .state import _mark_dirty, get_working_ontology, is_dirty, load_ontology, ontology_version, relation_items, save_to_disk
...
def _safe_ontology_name() -> str:
    try:
        ont = load_ontology()
        return str(ont.get("ontology_name", "Ontology graph"))
    except Exception:
...
@app.route("/")
def index() -> str:
    return render_html(_safe_ontology_name(), api_base="")
